...
<head>
    <meta charset="UTF-8">
    <title>Detail Pendaftaran: {{ $registration->user->nama }}</title>
...
<body class="dashboard-main" style="padding-top: 30px;">
    <div class="dashboard-container">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
             <div><h2>Detail Pendaftaran Organisasi</h2><p style="margin: 5px 0 0; color: #777;">Tinjau data pendaftar dan perbarui status.</p></div>
        ...
        <div class="detail-grid">
            <div class="main-content">
                <div class="card">
                    ...
                    <div class="detail-item"><strong>Email:</strong> <span>{{ $registration->user->email }}</span></div>
                    <div class="detail-item"><strong>Telepon:</strong> <span>{{ $registration->user->telepon }}</span></div>
                </div>
                <div class="card">
                    <h3><i class="fas fa-tasks"></i> Pilihan Divisi & Motivasi</h3>
                ...
                <div class="card">
                    ...
                    <div class="detail-item"><strong>Status Saat Ini:</strong> <span class="status-badge status-{{ $registration->status }}">{{ $registration->status }}</span></div>
                     @if($registration->status == 'approved' && $registration->accepted_division)
                        <div class="detail-item"><strong>Diterima di:</strong> <span style="font-weight: bold; color: #2E7D32;">{{ $registration->accepted_division }}</span></div>
                    @endif
                    <hr style="margin: 20px 0;">
                    
                    <form action="{{ route('admin.registration.update', ['type' => 'organization', 'id' => $registration->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <label for="status" style="display:block; margin-bottom:10px;"><strong>Ubah Status:</strong></label>
                        <select name="status" id="status" class="form-select-registration" required onchange="toggleDivisionSelect(this.value)">
                        ...
                        <div id="division-select-wrapper" style="display: none; margin-top: 15px;">
                            <label for="accepted_division" style="display:block; margin-bottom:10px;"><strong>Tempatkan di Divisi: *</strong></label>
                            <select name="accepted_division" id="accepted_division" class="form-select-registration">
                                <option value="">-- Pilih Divisi --</option>
                                @if($registration->division_1)<option value="{{ $registration->division_1 }}" {{ $registration->accepted_division == $registration->division_1 ? 'selected' : '' }}>Pilihan 1: {{ $registration->division_1 }}</option>@endif
                                @if($registration->division_2)<option value="{{ $registration->division_2 }}" {{ $registration->accepted_division == $registration->division_2 ? 'selected' : '' }}>Pilihan 2: {{ $registration->division_2 }}</option>@endif
                                @if($registration->division_3)<option value="{{ $registration->division_3 }}" {{ $registration->accepted_division == $registration->division_3 ? 'selected' : '' }}>Pilihan 3: {{ $registration->division_3 }}</option>@endif
                            </select>
                        </div>
    ...
    <script>
        function toggleDivisionSelect(status) {
            const wrapper = document.getElementById('division-select-wrapper');
            const select = document.getElementById('accepted_division');
            if (status === 'approved') {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            toggleDivisionSelect(document.getElementById('status').value);
        });
    </script>
</body>
</html>